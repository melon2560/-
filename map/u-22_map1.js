...
let cpuScore = parseInt(localStorage.getItem('cpscore'), 10) || 0; // CPUのスコア
...
dialogButton.addEventListener('click', () => {
    ...
    localStorage.setItem('cpscore', cpuScore);
...
function updateCPUScore() {
    // プレイヤーのスコアに基づいてCPUスコアを設定
    const randomFactor = Math.floor(Math.random() * 5) - 10; // -2から+2のランダムな値
    cpuScore = Math.max(0, score + randomFactor); // スコアが負にならないように調整
    document.getElementById('cpuScore').textContent = `CPUスコア: ${cpuScore}`;
    //console.log(`CPUのスコア: ${cpuScore}`);
}
